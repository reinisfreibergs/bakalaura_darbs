...
model_path = f'./results/v5_1/run-22-03-21--09-17-20/model_test_{SEQUENCE_LENGTH}_test_loss.pt'
...
start = start.expand(args.batch_size, SEQUENCE_LENGTH, 2)
start_unused = start
...
with torch.no_grad():
    for i in tqdm(range(FRAMES)):
        ...
        diffs = torch.cat(tensors=(diffs, diffs_current[0,-1,:].unsqueeze(dim=0)), dim=0)
